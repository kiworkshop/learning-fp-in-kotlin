...
class Chapter4 : StringSpec({
    "Exercise 1: invokeOrElse / orElse" {
        class PartialFunction<P, R>(
            ...
            fun orElse(that: PartialFunction<P, R>): PartialFunction<P, R> = PartialFunction({
                when {
                    this.isDefinedAt(it) || that.isDefinedAt(it) -> true
                    else -> false
                }
            }, {
                when {
    ...
    "Exercise 3: 두개의 매개변수를 받아서 큰 값을 반환하는 max 함수를 커링을 사용해 구현" {
        fun curriedMax(i: Int): (Int) -> (Int) -> (Int) = { p1 ->
            { p2 -> kotlin.math.max(p1, p2) }
        }
    ...
    "Exercise 7: 리스트에 조건 함수 적용하여 참인 요소의 리스트를 반환" {
        tailrec fun <P1> takeWhile(evaluate: (P1) -> Boolean, list: List<P1>, acc: List<P1> = listOf()): List<P1> =
            when {
                list.isEmpty() -> acc
                else -> {
                    takeWhile(
                        evaluate,
                        list.tail(),
                        acc + if (evaluate(list.head())) listOf(list.head()) else listOf()
                    )
                }
            }

        fun compareFunc(i: Int): Boolean = i < 3
        takeWhile(::compareFunc, listOf(1, 2, 3, 4, 5)) shouldBe listOf(1, 2)
    }

    "Exercise 8: sequence로 바꺼서 구현" {
