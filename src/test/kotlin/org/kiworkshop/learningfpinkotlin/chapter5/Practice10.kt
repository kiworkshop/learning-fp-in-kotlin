...
class Practice10 : FreeSpec() {
    ...
    init {
        "filter 함수를 foldLeft 함수를 사용해서 다시 작성해보자."{
            myFunListOf(1, 2, 3).filterFoldLeft { it % 2 == 0 } shouldBe myFunListOf(2)
            myFunListOf(1, 2, 3, 4).filterFoldLeft { it % 2 != 0 } shouldBe myFunListOf(1, 3)
            emptyMyFunList<Int>().filterFoldLeft { it % 2 == 0 } shouldBe emptyMyFunList()
            myFunListOf(1, 2, 3, 4, 5, 6, 7).filterFoldLeft { it < 5 } shouldBe myFunListOf(1, 2, 3, 4)
        }
    }
}
