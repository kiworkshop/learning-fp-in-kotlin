...
class Chapter8 : StringSpec({
    ...
    "Example 8-1" {
        ...
        val partiallyAppliedFunc1: Functor<(Int) -> Int> = funList.fmap { x -> { y -> x * y } }
        val partiallyAppliedFunc2 = funList.fmap { x: Int -> { y: Int -> x + y } }

        partiallyAppliedFunc1.fmap { x -> x(5) } shouldBe funListOf(5, 10, 15, 20)
        partiallyAppliedFunc2.fmap { x -> x(2) } shouldBe funListOf(3, 4, 5, 6)
    }
